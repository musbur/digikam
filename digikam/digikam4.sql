...
CREATE TRIGGER move_tagstree UPDATE OF pid ON Tags
                ...
                END;
CREATE VIEW v_video AS SELECT
    AlbumRoots.id AS root_id,
    AlbumRoots.label AS label,
    AlbumRoots.specificPath AS root,
    Albums.relativePath AS album,
    Images.name AS name,
    Images.uniqueHash AS hash
    FROM AlbumRoots
        JOIN Albums ON AlbumRoots.id=Albums.albumRoot
        JOIN Images ON Albums.id=Images.album
        JOIN VideoMetaData ON VideoMetaData.imageid=Images.id;
